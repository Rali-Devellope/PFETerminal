...
const NAV_ITEMS = [
  { to: '/encadrant', end: true, label: 'Vue d\'ensemble',
  ...
  return (
    <DashboardLayout navItems={NAV_ITEMS}>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Suivi de vos étudiants encadrés</p>
      </div>

      <div className="grid gap-4 sm:grid-cols-3 mb-6">
        <StatCard label="Étudiants encadrés" value={pfesArr.length} color="#2db84b"
        ...
        <StatCard label="PFE en cours" value={enCours} color="#0ea5e9"
        ...
        <StatCard label="PFE terminés" value={termines} color="#7e22ce"
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {/* Liste étudiants */}
        <Card title={`Mes étudiants (${pfesArr.length})`}
          ...
          {!pfesLoading && pfesArr.length === 0 && (
            <p className="text-sm text-gray-400 py-4 text-center">Aucun étudiant encadré</p>
          )}
          {!pfesLoading && pfesArr.length > 0 && (
              ...
              {pfesArr.map((p) => (
                <div key={p.id} className="py-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {p.sujet?.filiere && (
                      <p className="text-xs text-gray-400">{p.sujet.filiere}</p>
                    )}
                  </div>
        ...
        {/* Soutenance à venir */}
        <Card title="Prochaine soutenance"
            ...
            <div className="flex items-start gap-4 p-3 rounded-xl" style={{ backgroundColor: '#faf5ff' }}>
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">
                  {new Date(prochaine.date).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })} · {prochaine.salle} · {prochaine.duree} min
                </p>
        ...
        <Card title="Soutenances terminées"
