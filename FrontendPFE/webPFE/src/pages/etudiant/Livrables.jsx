...
export default function EtudiantLivrables() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setSuccessMsg('Livrable déposé avec succès !')
      setErrorMsg('')
  ...
  const handleSubmit = (e) => {
    ...
    if (!pfeId) { setErrorMsg('PFE introuvable'); return }
  ...
  return (
    <DashboardLayout navItems={NAV_ITEMS}>
      <div className="mb-6">
        <h1 className="text-2xl font-bold tracking-tight" style={{ color: '#1a2744' }}>Mes Livrables</h1>
        <p className="text-sm text-gray-500 mt-1">Déposez et consultez vos fichiers PFE</p>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Type */}
            <div>
              <label className="block text-xs font-semibold mb-2 text-gray-600">Type de livrable</label>
              <div className="flex gap-2">
                {TYPE_OPTIONS.map((t) => (
                  <button
                    key={t.value}
                    type="button"
                    onClick={() => setForm({ ...form, type_livrable: t.value })}
                    className="flex-1 py-2 rounded-xl text-xs font-medium transition"
                    style={form.type_livrable === t.value
            ...
            {/* Zone drag & drop */}
            <div>
              <label className="block text-xs font-semibold mb-2 text-gray-600">Fichier</label>
              <div
              ...
              >
                <input
                  ...
                  type="file"
                  className="hidden"
                  accept=".pdf,.zip,.pptx,.docx,.ppt,.doc"
                ...
                {selectedFile ? (
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-2">
                      Glissez un fichier ici ou <span style={{ color: '#2db84b' }}>cliquez pour parcourir</span>
                    </p>
                    <p className="text-xs text-gray-400 mt-1">PDF, ZIP, PPTX, DOCX</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="Ajoutez une note pour l'encadrant..."
            ...
            <button
              type="submit"
              disabled={mutation.isPending || !pfeId}
              ...
              style={{
                background: mutation.isPending || !pfeId
        ...
        {/* Historique */}
        <Card
          title={`Historique${livrables.length > 0 ? ` (${livrables.length})` : ''}`}
