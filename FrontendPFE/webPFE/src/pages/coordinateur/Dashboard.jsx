...
export default function CoordinateurDashboard() {
  ...
  return (
    <DashboardLayout navItems={NAV_ITEMS}>
      <div className="mb-6">
        <h1 className="text-2xl font-bold tracking-tight" style={{ color: '#1a2744' }}>{t('coordinateur.dash_title')}</h1>
        <p className="text-sm text-gray-500 mt-1">{t('coordinateur.dash_sub')}</p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        <StatCard label={t('coordinateur.total_pfe')} value={stats.total_pfe ?? '—'} color="#2db84b"
        ...
        <StatCard label={t('coordinateur.sujets_attente')} value={stats.sujets_en_attente ?? sujetsArr.length} color="#f59e0b"
        ...
        <StatCard label={t('coordinateur.souts_planif')} value={stats.soutenances_planifiees ?? soutsArr.length} color="#7e22ce"
        ...
        <StatCard label={t('coordinateur.moyenne')} value={stats.moyenne_generale ? `${stats.moyenne_generale}/20` : '—'} color="#0ea5e9"
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card title={t('coordinateur.sujets_valider', { count: sujetsArr.length })}
          action={<Link to="/coordinateur/sujets" className="text-xs font-medium px-2.5 py-1 rounded-lg" style={{ backgroundColor: '#fef3c7', color: '#b45309' }}>{t('coordinateur.voir_tout')}</Link>}
        ...
        <Card title={t('coordinateur.pfe_recents')}
          action={<Link to="/coordinateur/sujets" className="text-xs font-medium px-2.5 py-1 rounded-lg" style={{ backgroundColor: '#f0fdf4', color: '#15803d' }}>{t('coordinateur.all_pfe')}</Link>}
          ...
          {pfesArr.length === 0
            ? <p className="text-sm text-gray-400 py-2">{t('coordinateur.no_pfe')}</p>
            : <div className="space-y-2">
                {pfesArr.map((p) => (
                  <div key={p.id} className="flex items-center justify-between p-3 rounded-xl" style={{ backgroundColor: '#f8fafc' }}>
                    <div className="min-w-0">
                      <p className="text-sm font-semibold truncate" style={{ color: '#1a2744' }}>{p.etudiant?.prenom} {p.etudiant?.nom}</p>
        ...
        <Card title={t('coordinateur.souts_prochaines')}
          action={<Link to="/coordinateur/soutenances" className="text-xs font-medium px-2.5 py-1 rounded-lg" style={{ backgroundColor: '#faf5ff', color: '#7e22ce' }}>{t('coordinateur.planifier_link')}</Link>}
          ...
          {soutsArr.length === 0
            ? <p className="text-sm text-gray-400 py-2">{t('coordinateur.no_souts')}</p>
            : <div className="grid sm:grid-cols-2 gap-2">
                {soutsArr.map((s) => (
                  <div key={s.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ backgroundColor: '#faf5ff' }}>
                    <div className="w-10 h-10 rounded-xl flex flex-col items-center justify-center flex-shrink-0 text-center" style={{ backgroundColor: '#ede9fe' }}>
                      <span className="text-xs font-bold" style={{ color: '#7e22ce' }}>{new Date(s.date).getDate()}</span>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400">{s.salle} · {s.duree} {t('common.min')}</p>
                    </div>
                    <Badge statut={s.statut} />
                  </div>
                ))}
